...
# there, with every `openspec` command told to run from `mia-docs/`. Nothing
# else differs: project specifics live in mia-docs/openspec/config.yaml, which
# the generated skills read at run time. Refresh: `cd mia-docs && openspec
# update`, then this, then review the diff. Codex's UI metadata
# (`agents/openai.yaml`) is kept by hand and left alone; a skill that vanished
# upstream is removed here too.
...
openspec-skills:
  ...
  for src_root, dst_root in (('mia-docs/.claude/skills', '.claude/skills'), ('mia-docs/.agents/skills', '.agents/skills')):
      ...
      for name in names:
          ...
          if n == 0:
              raise SystemExit(name + ': no openspec command found to prefix — the upstream text changed shape, adjust this recipe')
