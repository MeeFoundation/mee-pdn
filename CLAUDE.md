...
- [`crates/data-layer`](crates/data-layer/) — the data layer over the forked iroh-docs (git dependency on `github.com/MeeFoundation/pdn-store`, capability-gated ingest per ADR-0008): the entries-only `DataLayer` trait, `SyncNode` stack assembly, the `Binding` registry (`Data` namespaces vs the device-shared `Connections` store), the `IngestPolicy` gate (`SelfOwned` device axiom + `AnyOf` + the naive in-memory `ConnectionsPolicy`), and `ConnectionsStore` (an identity's connections replicated between its devices). Scenario tests in its `tests/`. Capability _semantics_ stay above: tokens are opaque payloads here, policies are injected. To hack on the fork locally, `[patch]` it to `../pdn-store` (see the workspace `Cargo.toml` comment).
