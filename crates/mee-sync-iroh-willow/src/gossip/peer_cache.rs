...
pub struct CachedPeer {
    ...
    /// Whether we have an active sync connection.
    pub connected: bool,
}
...
impl PeerCache {
    ...
    pub fn upsert(&mut self, ad: PeerAdvertisement) -> bool {
        ...
        if let Some(cached) = self.peers.get(&ad.peer_id) {
            ...
            let connected = cached.connected;
            self.peers.insert(
                ad.peer_id,
                CachedPeer {
                    advertisement: ad,
    ...
    pub fn set_connected(&mut self, peer_id: &[u8; 32], connected: bool) {
        if let Some(cached) = self.peers.get_mut(peer_id) {
    ...
    pub fn all_peers(&self) -> Vec<CachedPeerInfo> {
        self.peers
            ...
            .map(|c| CachedPeerInfo {
                ...
                version: c.advertisement.version,
...
mod tests {
    ...
    fn make_ad(peer_id: [u8; 32], version: u64) -> PeerAdvertisement {
