...
- **Test** (nextest): `just test -p pdn-store` runs the default feature set; `-E 'test(test_name)'` selects one test. `just test-store` adds `--all-features`, `--no-default-features`, and the doctests — nextest runs none, and the README example is one. The tests carry `#[ignore = "flaky"]` and run only in the nightly workflow; `just stress -p pdn-store --run-ignored ignored-only` runs them by hand.
...
**The sync actor** (`actor.rs`): `SyncHandle` is a cheaply-cloneable handle to a dedicated **`std::thread`** (`"sync-actor"`) that owns the `Store` and processes `Action` messages sequentially. It is a thread, not a tokio task, because **redb is blocking** — but on `wasm_browser` it falls back to a tokio task. All replica mutation goes through here; the last handle drop joins the thread. On shutdown the actor closes and empties its channel: a request queued behind the shutdown would otherwise keep its reply open for as long as any handle lives, and its caller would wait forever. Prefer `SyncHandle::shutdown().await` over relying on drop to avoid blocking an async context.

**The live engine** (`engine/`): `Engine` ties the `SyncHandle`, a `LiveActor`, and a per-document gossip swarm together. `live.rs` is the coordinator — it serves sessions the protocol handler has already read, drives syncs, and reacts to gossip `Op`s (`Put` / `ContentReady` / `SyncReport`) by triggering blob downloads. `state.rs` tracks per-namespace sync state (`Origin`, `SyncReason`); `gossip.rs` manages the swarm. The engine also installs a GC-protect callback so blobs referenced by docs aren't garbage-collected by `iroh-blobs`. An engine holds nothing of the node it runs in but the sending end of `CoLocatedRequests`, the channel it hands local-write announcements and dials to this node's own endpoint to, and `Docs` owns the RPC actor serving its API: the actor holds the engine and an API handle can sit inside what the engine holds, so the last `Docs` dropped stops the actor and lets the engine go. A callback into the node that owned something of it would close a cycle through the engine.

**Networking** (`net.rs` + `net/codec.rs`): ALPN is `/iroh-sync/1`. `connect_and_sync` is the initiator ("Alice"), `handle_session` the responder ("Bob"); `codec.rs` holds the wire state machines (`run_alice`, `BobState`) that exchange `ranger::Message`s over an iroh QUIC bi-stream. An accepted connection is read by the protocol handler — `accept_session`, then `handle_session` or `refuse_session` — never by the engine: an engine takes a session whose first message is already read, so no accept path waits on the wire inside the live actor's loop. That first message is read under `MAX_OPENING_FRAME`, before anything classifies the caller, and the session ceiling applies only once `SessionOpening::into_parts` hands the reader on; the opening's size is bounded by `MAX_KEY_BYTES`, which `validate_entry` enforces on every local write and at ingest.
...
- A read across authors orders entries as `put` does within one: `LatestPerKeySelector::push` picks the greater `(timestamp, content_hash)`, so equal timestamps resolve by content hash, as the data layer's stores specify (`data-store`, `connection-metadata-store`, `private-metadata-store`). Upstream compares the timestamp alone and leaves the first author in key order, so an upstream patch touching the selector is adapted.
- No actor waits on a subscriber outside the crate: `subscribers.rs` gives each subscription a `Delivery`, and every one opened through the public interface is `Lossy` — a full channel drops the event and a `Lagged` follows the last one received. Only the live actor subscribes to a replica `Blocking`, through the crate-only `OpenOpts::subscribe`. Upstream waits on every subscriber, so a subscriber that stopped reading stopped the actor and every replica it serves; an upstream patch touching `Subscribers`, `Event` or `LiveEvent` is adapted. The contract is specified in the data layer's `change-subscription` spec.
