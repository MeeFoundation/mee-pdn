...
2. **Determine the workflow schema**
   ...
   **Use a different schema only if the user:**
   ...
   - Asks to "show workflows" or asks "what workflows" exist → resolve the authoritative root by running `cd mia-docs && openspec context --json` from the current working directory. If the user explicitly selected a registered store, use `cd mia-docs && openspec context --json --store "<store-id>"`. Then run `cd mia-docs && openspec schemas --json` with its working directory set to the returned `root.path` and let them choose. This preserves roots selected by a local `store:` pointer or the global `defaultStore`; when a registered store was explicitly selected, append `--store "<store-id>"` to `cd mia-docs && openspec schemas --json` as well. If context reports only `no_openspec_root`, run `cd mia-docs && openspec schemas --json` from the current working directory instead. Do not use this fallback for invalid or unavailable stores.
...
3. **Create the change directory**
   ...
   ```bash
   cd mia-docs && openspec new change "<name>" --schema "<schema-name>"
   ```
...
5. **Create every artifact in the required set**
   ...
   a. **For each artifact that is `ready` (dependencies satisfied)**:
      - Get instructions:
        ```bash
        cd mia-docs && openspec instructions <artifact-id> --change "<name>" --json
        ```
   ...
   b. **Continue until every artifact in the required set exists (not just `apply.requires`)**
      - After creating each artifact, re-run `cd mia-docs && openspec status --change "<name>" --json`
...
6. **Show final status**
   ```bash
   cd mia-docs && openspec status --change "<name>"
   ```
...
- Follow the `instruction` field from `cd mia-docs && openspec instructions` for each artifact type - it is the authoritative guidance, even for familiar artifact names
