...
1. **Select the change**
   ...
   **Load current archive inputs before the existing archive checks:**
   ...
   ```bash
   cd mia-docs && openspec instructions archive --change "<name>" --json
   ```
...
2. **Check artifact completion status**
   ...
   Run `cd mia-docs && openspec status --change "<name>" --json` to check artifact completion.
...
4. **Assess delta spec sync state**
   ...
   **Prompt options:**
   ...
   `cd mia-docs && openspec instructions specs --change "<name>" --json` once with the same
...
5. **Perform the archive**
   ...
   Generate the target name: use the change name as-is when it already starts with a `YYYY-MM-DD-` prefix; otherwise prepend the current date as `YYYY-MM-DD-<change-name>`. Never stack a second date (same rule as `cd mia-docs && openspec archive`).
...
- Use artifact graph (cd mia-docs && openspec status --json) for completion checking
