...
const CLAIM_SCHEMA = {
  ...
  properties: {
    findings: {
      ...
      items: {
        type: 'object',
        required: ['title', 'file', 'line', 'severity', 'reach', 'origin', 'symptom', 'cause', 'victim', 'evidence', 'fixes', 'recommendation'],
        properties: {
          title: { type: 'string' },
          file: { type: 'string' },
          line: { type: 'string' },
...
const grouped = ((triage && triage.groups) || [])
  .map((g) => {
    ...
    return {
      ...
      reach: members.map((m) => m.reach).sort((a, b) => (REACH[a] ?? 9) - (REACH[b] ?? 9))[0] || base.reach,
