...
- **The review is written in the language resolved in §0**, English by default. Never translated, whatever the language: capability, connection metadata store / CMS, private metadata store / PMS, claim, lock, race, identity, audience, connection, binder, session, snapshot, ingress, egress. Identifiers, paths, and type names in backticks, exactly as in the code.
